...
research. \allison{NEW} Thus, although more traditional statistics coursework has been 
incorporated into environmental science programs, often these fail to arm 
students with the data science skills necessary for research. A ``symptom of the
...
Carpentry's workshops teach domain-specific lessons covering the ``fundamental
data skills'' needed for the ``full lifecycle of data-driven research,'' the
possibility of further tailoring these workshops to populations of graduate
student researchers has yet to be investigated.
...
data modeling is but one component. \allison{NEW} We focus on the ``data science skills'' 
...
% \href{http://bit.ly/ws_recordings}{MSU Library
% videos (link)}}.
...
\textbf{ggplot2} (\citeyear{ggplot}). The common syntax of these packages lessen
...
the use of conditional statements, stepping from \texttt{if()\{\}}, to 
\texttt{if()\{\}else\{\}}, to \texttt{else if()\{\}} statements. Subsequently,
the use of the \texttt{ifelse()} function is motivated by the need to vectorize
these types of conditional operations. The second half of the workshop covers
...
replicate the same set of instructions. Working through exercises which repeat
operations on a dataset using both a \texttt{for()} loop and a recursive
\texttt{for()} loop, motivates a discussion of why vectorization is recommended
for non-recursive \texttt{for()} loops. \allison{NEW} In the interest of time, participants
are provided with resources to continue learning about vectorized operations
using the \texttt{purr} package. 

\todo{How to get the curly braces to not be larger than the parentheses?}
