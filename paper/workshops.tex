...
from the Ecology, Land Resources and Environmental Sciences, Animal and Range 
Sciences, and Plant Sciences and Plant Pathology departments were emailed 
...
sentiments were voiced alongside students' need to perform other data wrangling
...
faculty that this theme captures were not voiced alongside a specific software.
...
catching up to the next generation.'' These gaps impact the assistance faculty
can provide to their students, as ``increasingly faculty feel that they're not
at the forefront of their programming abilities, so their students are being 
...
same set of instructions many times. Looping, specifically \texttt{for()} loops, 
are introduced as a popular way to iterate or replicate the same set of
instructions many times. Participants work through examples of a \texttt{for()}
loop and a recursive \texttt{for()} loop, in the context of repeated operations
on a dataset. These exercises motivate a discussion of why \texttt{R} users
recommend the use of vectorization for non-recursive \texttt{for()} loops. Next,
functions are presented as an approach to replicate the same set of instructions
in multiple locations throughout your code. Persuaded by a script which copies
and pastes the same process multiple times, participants understand the
difficulty in discerning the underlying process and spotting mistakes.
Subsequently, participants are tasked with transforming this copy-paste-modify
process into a function. By parsing out the function writing process into a set
of steps that one walks through once you've copied and pasted your code multiple
times, participants have a more intuitive sense for why functions are useful and
how they can create them in their own code.
...
table's rows with \texttt{arrange()}. Prompted by the need to perform a sequence
...
geoms to create violin plots, density plots, bar charts, and line plots. These
plots allow participants to explore different \texttt{geom} functions
...
\quad These tailored workshops were offered during the 2018-2019 academic year.
Each workshop lasted a total of three hours. The \emph{Introduction to
\texttt{R}} and \emph{Intermediate \texttt{R}} workshops were offered both 
semesters, and the \emph{Data Wrangling} and \emph{Data Visualization} workshops
were only offered in the spring. The workshop series began with 
\emph{Introduction to \texttt{R}}, followed by \emph{Intermediate \texttt{R}}, 
\emph{Data Wrangling}, and \emph{Data Visualization}, with three weeks between 
workshops. In the week prior to each workshop, a pre-workshop survey is
...
\quad During the 2018-2019 academic year, a total of 150 students, faculty, and
staff attended at least one of the workshops, and we obtained 121 complete
survey responses. Of these responses, a total of 64 individuals attended the 
\emph{Introduction to \texttt{R}} workshop, 40 attended \emph{Intermediate
\texttt{R}}, and 9 attended each of the \emph{Data Wrangling} and \emph{Data
Visualization} workshops. The majority of the workshop attendees were from 
environmental science fields---from departments such as Land Resources and
Environmental Sciences (LRES), Ecology, Plant Sciences and Plant Pathology, 
Biochemistry or Microbiology, Animal and Range Sciences, and Earth Sciences.
Additionally, over 60\% of workshop attendees were masters and doctoral
students. It is worth noting, however, that 18 faculty, staff, and postdocs also
attended these workshops. Figure \ref{fig:departments} displays the department
affiliations of the workshop attendees and their current occupation. 
...
participants, 30\% of \emph{Intermediate \texttt{R}} participants, 80\% of  
\emph{Data Wrangling} participants, and 50\% of \emph{Data Visualization} 
participants stating the information was new to them. Across every
...
researchers.'' A number of graduate students reflected on the importance
...
computational approaches to extract meaning from data, [and] program in
higher-level languages" \citep[p. 7]{asa}. While we may see these changes
reflected in undergraduate and graduate programs in statistics, integrating
these topics into graduate-level statistics service courses has received less
attention and poses different issues. 
